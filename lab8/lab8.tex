...
\subsection{Автоколебательный генератор}
...
При установке $E_{01} = 0.7 E_\text{0\ ном}$:
...
\noindent Минимальное напряжение запуска: $U_\text{зап\ min} = 3.8$ В\\
Минимальное время запуска: $t_\text{зап\ min} = 12$ мкс\\
Время восстановления: $t_\text{восст} = 6.7$ мкс\\
